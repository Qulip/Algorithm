...
public class Main {
    ...
    static void dfs(int r, int c, int idx, int score){
        if(idx == 4){
            rst = rst > score ? rst : score;
            return;
        }
        for(int i=0; i<4; i++){
            if(r+d[i][0]<0||r+d[i][0]>=map.length||c+d[i][1]<0||c+d[i][1]>=map[0].length||chk[r+d[i][0]][c+d[i][1]]) continue;
            chk[r][c] = true;
            dfs(r+d[i][0], c+d[i][1], idx+1, score + map[r][c]);
            chk[r][c] = false;
        }
    }

    static int chk_U(int r, int c){
        int sum = 0, tmp = 0;
        if(r-2>=0) {
            if (c + 1 < map[0].length) sum = map[r][c] + map[r - 1][c] + map[r - 1][c + 1] + map[r - 2][c];
            if (c - 1 >= 0) tmp = map[r][c] + map[r - 1][c] + map[r - 1][c - 1] + map[r - 2][c];
            sum = sum > tmp ? sum : tmp;
        }
        if(c+2<map[0].length){
            tmp = 0;
            if(r+1< map.length) tmp = map[r][c] + map[r][c+1] + map[r+1][c+1] + map[r][c+2];
            sum = sum > tmp ? sum : tmp;
            tmp = 0;
        ...
        if(r+2< map.length) {
            tmp = 0;
            if (c + 1 < map[0].length) tmp = map[r][c] + map[r + 1][c] + map[r + 1][c + 1] + map[r + 2][c];
            sum = sum > tmp ? sum : tmp;
            tmp = 0;
            if (c - 1 >= 0) tmp = map[r][c] + map[r + 1][c] + map[r + 1][c-1] + map[r + 2][c];
            sum = sum > tmp ? sum : tmp;
